...
private:
  ...
  OverlappingWFC(
    ...
    std::cout << "Pattern Size: " << options.pattern_size << std::endl;
    std::cout << "Periodic Output: " << options.periodic_output << std::endl;
    std::cout << "Periodic Input: " << options.periodic_input << std::endl;
    std::cout << "Ground: " << options.ground << std::endl;
    std::cout << "Symmetry: " << options.symmetry << std::endl;
    std::cout << "Output Width: " << options.out_width << std::endl;
    std::cout << "Output Height: " << options.out_height << std::endl;
  }
...
std::optional<unsigned int> get_pattern_id(const Array2D<T>& pattern) {
    auto it = std::find(patterns.begin(), patterns.end(), pattern);
    // Check if the pattern was found by comparing the iterator to the end of the vector.
    if (it != patterns.end()) {
        // Calculate the index of the found pattern.
        unsigned int index = std::distance(patterns.begin(), it);
        return index;  // Return the index as the pattern ID.
    } else {
        return std::nullopt;  // Return nullopt if the pattern was not found.
    }
}
...
bool set_pattern(const Array2D<T>& pattern, unsigned i, unsigned j) noexcept {
    //std::swap(i, j);  // Swap the i and j coordinates to match the expected order of the WFC algorithm (x, y
    auto pattern_id_opt = get_pattern_id(pattern);
    if (!pattern_id_opt.has_value() || i >= options.get_wave_height() || j >= options.get_wave_width()) {
        return false;
    }
    unsigned pattern_id = pattern_id_opt.value();
    set_pattern(pattern_id, i, j); // Call the private overload with the extracted pattern_id.
    return true;
}
...
// Changes:
// fixed get_pattern_id()
// Coordinate Swapping in set_pattern()
